...
def expandMonthly(dfProjects: pd.DataFrame, months: int = 12, defaultRate: float = 22.0) -> pd.DataFrame:
    end = pd.Timestamp.today().to_period("M")
    monthsIdx = pd.period_range(end - (months - 1), end, freq="M")
    rows = []
    for _, r in dfProjects.iterrows():
        rate = float(r["hourlyRate"]) if "hourlyRate" in dfProjects.columns else float(defaultRate)
        for m in monthsIdx:
